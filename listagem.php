...
if( isset($_GET["nome"])){
    $nome = $_GET["nome"];
    $comando = $pdo->prepare("SELECT * FROM dados1 where nome='$nome'");
}else{
    $comando = $pdo->prepare("SELECT * FROM dados1");
}
                ...
                <table border="1" class="usuariot">
                    ...
                    <?php
                        while ($linhas = $comando->fetch()){
                            $data = $linhas["data"];
                            $sexo = $linhas["sexo"];
                            $hospital = $linhas["hospital"];
                            $nome = $linhas["nome"];
                            $idade = $linhas["idade"];
                            $rg= $linhas["rg"];
                            $phone= $linhas["phone"];
                            $local_ocorrencia= $linhas["local_ocorrencia"];
                            $acompanhante= $linhas["acompanhante"];
                            ...
                            echo("
                                <tr>
                                    ...
                                    <td>
                                    <img src='image/lapis.png' width='25px' onclick=\"Editar('$data','$sexo','$hospital','$nome','$idade','$rg','$phone','$local_ocorrencia','$acompanhante','$nome_acompanhante','$idade_acompanhante');\" >
                                    </td>
                                </tr>
                            ");
                        }
